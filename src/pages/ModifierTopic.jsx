...
import { 
  allModifierQuestions, 
  getUniqueYears, 
  getUniqueBoards, 
  getModifierQuestionsByFilters,
  getQuestionsByRuleId 
} from '../data/topics/modifier/questionLoader';
...
const ModifierTopic = () => {
  ...
  const filteredQuestions = useMemo(() => {
    return getModifierQuestionsByFilters(selectedYear, selectedBoard).filter(q => {
  ...
  const categorizedQuestions = useMemo(() => {
    ...
    const questionsToProcess = getModifierQuestionsByFilters(selectedYear, selectedBoard);
  ...
  const ruleFrequency = useMemo(() => {
    const frequency = {};
    allModifierQuestions.forEach(q => {
      if (q.blanks) {
        q.blanks.forEach(blank => {
          if (blank.ruleId) {
  ...
  const sortedRules = useMemo(() => {
    if (sortRulesByFrequency) {
      return [...modifierRules].sort((a, b) => (ruleFrequency[b.id] || 0) - (ruleFrequency[a.id] || 0));
    }
  ...
  const totalQuestions = getModifierQuestionsByFilters(selectedYear, selectedBoard).length;
  ...
  const selectedRuleData = selectedRule ? modifierRules.find(r => r.id === selectedRule) : null;
  ...
  const renderPassageWithBlanks = (question) => {
    ...
    parts.forEach((part, index) => {
      ...
      if (index < parts.length - 1 && question.blanks && question.blanks[index]) {
        ...
        result.push(
          <span key={`blank-${index}`} className="inline-flex items-center">
            <span className="mx-1">
              <RuleTooltip ruleId={blank.ruleId}>
                <button
                ...
                >
                  {isAnswerVisible ? (
                    <>
                      <span className="mr-2">{blank.answer}</span>
                      <EyeOff className="w-3 h-3" />
                    </>
                  ) : (
                    <>
                      <span className="mr-2">({blank.id})</span>
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Onushilon Hub - Modifier</h1>
                  <p className="text-sm text-gray-600 dark:text-gray-400">Interactive modifier exercises with grammar rules (2016-2024)</p>
                </div>
              </div>
            </div>
            <div className="flex items-center space-x-3">
          ...
          <div className="lg:col-span-1">
            <div className="sticky top-6">
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-lg font-semibold text-gray-900 dark:text-white">Modifier Rules</h2>
                <button
                ...
                {sortedRules.map((rule) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <p className="text-xs text-gray-600 dark:text-gray-400">{rule.structure}</p>
                    </div>
                  );
                })}
              </div>
